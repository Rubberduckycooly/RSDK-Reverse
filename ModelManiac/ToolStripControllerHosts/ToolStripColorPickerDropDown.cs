...
namespace ModelManiac
{
  ...
  internal class ToolStripColorPickerDropDown : ToolStripDropDown
  {
    ...
    {
      this.Host = new Cyotek.Windows.Forms.ColorGrid
                  {
                    AutoSize = true,
                    Columns = 10,
                    Palette = Cyotek.Windows.Forms.ColorPalette.Office2010
                  };
    ...
    [Browsable(false)]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    public Cyotek.Windows.Forms.ColorGrid Host { get; private set; }
    ...
    {
            Cyotek.Windows.Forms.ColorHitTestInfo info;

      info = this.Host.HitTest(e.Location);

      if (info.Index != Cyotek.Windows.Forms.ColorGrid.InvalidIndex)
      {
